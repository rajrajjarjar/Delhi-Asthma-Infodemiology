...
OUT_PATH = Path("E:/Projects/BTP/Data/Raw_data/2_google_trends_raw.csv")
...
def rescale_batch(batch_df: pd.DataFrame, reference_anchor: pd.Series) -> pd.DataFrame:
    ...
    IMPORTANT: if this batch's anchor is zero for every week, there is no
    valid ratio to rescale by. The old version divided-by-zero, replaced
    inf with 0, and ended up multiplying real data by 0 -- silently
    wiping out legitimate values. We now detect that case, SKIP rescaling,
    and keep the raw values instead, with a printed warning so you know
    those particular columns are on a different (unrescaled) 0-100 scale
    and should be visually sanity-checked before trusting them alongside
    rescaled columns.
    """
    anchor_col = batch_df[ANCHOR_TERM]
    other_cols = batch_df.drop(columns=[ANCHOR_TERM])

    if (anchor_col == 0).all():
        print(
            f"    WARNING: anchor term was 0 for every week in this batch — "
            f"cannot rescale. Keeping RAW (unrescaled) values for: {list(other_cols.columns)}"
        )
        return other_cols

    # Avoid divide-by-zero on individual weeks where anchor is 0 but not
    # every week is 0 -- fall back to a ratio of 0 for just that single
    # week (that week's value becomes 0 too, but doesn't nuke the batch).
    ratio = (reference_anchor /
             anchor_col).replace([float("inf")], 0).fillna(0)
    rescaled = other_cols.multiply(ratio, axis=0)
    return rescaled
